...
opt_prompt = "The system asks for the optional parameter {input_param}. The system's request must include all of the specified input parameters, as every mentioned parameter is needed. Do not use the word 'or'; instead, use 'and'."
fulled_prompt = "The system has already obtained the input parameter {input_param} as {value} from the user's utterance."
easy_prompt = "When the system requests an input parameter, it is important to refer to the description in parentheses as much as possible to clearly explain to the user what the parameter is."
...
user_action_list_backward = { 
    "inform_intent_clear":
        {"description":"Say what one wants specifically.",
         "template":"The user requests something from the system. The user says '{query}'."},
    ...
    "affirm":
        {"description":"Agree to the system's proposition.",
         "template":"The user agrees with the system's proposition."},
    "negate":
        {"description":"Deny the system's proposal.",
         "template":"The user denies the system's proposal."},
    "inform_intent_vague":
    ...
    "fail_inform":
        {"description":"Fail to reply to the system's request. In this turn, the user tells the system that they do not know the answer.",
         "template":"The system asks the user for the input parameter {input_param} to call API {api}, but the user fails to provide the input parameter for some reason. In this turn, the user says they do not know the information about the input parameter {input_param} and does not provide any input parameter information.",
         "template_tmp":"The user provided information about the input parameter {input_param} to the system. However, the user could not provide information about {chain_param} for some reason."},
    "inform_intent_re":
        {"description":"Re-request the previous request to the system under different conditions (different parameters).",
         "template":"The user re-requests to call {api} with different conditions, providing a different value for input parameter {input_param} as {value}."},
    "user_bye":
...
system_action_list_backward = {
    "clarify":
        {"description":"If the user's query is vague, re-ask the user to clarify their intent.",
         "template":"As a result of a proper API inquiry through the retriever, no API with a score of 0.6 or higher could be found. Accordingly, the system requests the user to clarify their intent."},
    "request":
        ...
        "template_chain":"Based on the retriever score, the system believes the results from API {api_s} can provide the input parameter {chain_param} for API {api_t}. To call {api_s}, the system asks the user for the input parameter. In this turn, the system does not provide an example input parameter to the user.",
        "template_tmp":"The system has collected information about the input parameter {chain_param} but still needs to collect the remaining input parameter {input_param}. The system then asks the user for information about {input_param}.",
        "template_add":"Based on the user's utterance, the system obtained the input parameter {chain_param} as {value}. The system still needs to ask the user for other input parameters."
        },
    "response":
    ...
    "response_fail":
        {"description":"Notify the user that the system cannot execute the request due to insufficient information.",
         "template":"The user did not provide the requested information to the system. Therefore, the system informs the user that it cannot resolve their query.",
    ...
    "call":
        {"description":"Call the API with the information collected from the user or elsewhere and do not respond to the user yet.",
         ...
         "template_add":"Based on the user's utterance, the system obtained the input parameter {input_param} as {value}.",
         "template_chain":"Based on the results of the retriever, the system believes the results '{output_comp}' from API {api_s} can provide the input parameter {chain_param} for API {api_t} and calls it."},
    "suggest":{
        "description":"Make a suggestion for an unclear user intent and ask whether it satisfies the user.",
        "template":"Since the user's query is unclear, no API with a retriever score above 0.6 has been found. However, several APIs have scores between 0.5 and 0.6. The system asks if it would be appropriate to run {api}, which has the highest score among them, and retrieve the result. At this time, the system does not mention the name of the API directly."
    },
    "retriever_call":{
...
class PairClass:
    ...
    def mock_data_prompt(self,subgraph,mock_data_dict):
    ...
    def generate_total_prompt(self,scenario_list):
        for scen in scenario_list:
            relation = self.relation(scen['subgraph'])
            tmp_prompt = self.prompt.format(common_instruction=self.common_instruction,relation=relation,scenario=scen['scenario'],data=self.mock_data_prompt(scen['subgraph'],scen['mock_data']))
    ...
    def is_same(self,param1,param2):
        if similarity_object(param1,param2)>0.83 and cosine_similarity(model.encode(param1),model.encode(param2))>0.77:
            return True

class ZeroYes(PairClass):
    def __init__(self,subgraph_list):
        ...
        self.action_seq_x = [ ## source를 한 번 거쳐야 함
            ("inform_intent_clear","retriever_call","request","fail_inform","retriever_call","call","call","response","user_bye","system_bye"), ## chain 빼고는 말해줌 #
            ("inform_intent_clear","retriever_call","request","fail_inform","retriever_call","call","request","inform","call","response","user_bye","system_bye"), ## chain을 모른다고만 함 #
            ("inform_intent_vague","retriever_call","clarify","inform_intent_clear","retriever_call","request","fail_inform","retriever_call","call","request","inform","call","response","user_bye","system_bye"), ## chain을 모른다고만 함 #
            ("inform_intent_vague","retriever_call","clarify","inform_intent_clear","retriever_call","request","fail_inform","retriever_call","call","call","response","user_bye","system_bye"), ## chain 빼고는 말해줌 #
            ("inform_intent_vague","retriever_call","suggest","affirm","request","fail_inform","retriever_call","call","request","inform","call","response","user_bye","system_bye"), ## chain을 모른다고만 함 #
            ("inform_intent_vague","retriever_call","suggest","affirm","request","fail_inform","retriever_call","call","call","response","user_bye","system_bye"), ## chain 빼고는 말해줌 #
            ("inform_intent_vague","retriever_call","suggest","negate","response_fail") #
        ]
        self.action_seq_x_multi = [ ## source 가 list 반환
            ("inform_intent_clear","retriever_call","request","fail_inform","retriever_call","call","request","inform","call","response","user_bye","system_bye"), ## chain 빼고는 말해줌 #
            ("inform_intent_vague","retriever_call","clarify","inform_intent_clear","retriever_call","request","fail_inform","retriever_call","call","request","inform","call","response","user_bye","system_bye"), ## chain 빼고는 말해줌 #
            ("inform_intent_vague","retriever_call","suggest","affirm","request","fail_inform","retriever_call","call","request","inform","call","response","user_bye","system_bye"), ## chain 빼고는 말해줌
            ("inform_intent_vague","retriever_call","suggest","negate","response_fail"),
        ]
        self.zero_yes_mid_o_list = []
        self.zero_yes_mid_x_list = []
        self.zero_yes_mid_x_multi_list = []
        self.freeform = []
        self.prompt_list = []
        self.scenario_list = []
        self.mockdata_list = []
    ...
    def zero_yes_mid_o(self): ## mid 가 아는 거
        for subgraph in self.subgraph_list:
            ...
            if len(source_params) != 0 or len(target_params) <=1:
                continue
            edge_param = edge[3][1]
            if not is_ben(edge_param): ## edge가 ben일 때
                continue
            self.zero_yes_mid_o_list.append(subgraph)
        print(f"Length of zerozero mid o:{len(self.zero_yes_mid_o_list)}")
    ...
    def is_output_in(self,chain,output_list):
        for output in output_list:
            if output == chain:
                ...
                if similarity_object(output,chain) >0.95:
                    return True
        return False
            
    def zero_yes_mid_x(self): ## mid 가 모르는 거
        for subgraph in tqdm(self.subgraph_list):
            ...
            for param in source_params:
                if similarity_object(param['object_ad'],edge_param)>0.83 or similarity_object(param['object_ad'],edge_output)>0.83:
                    is_same = True
            ...
            if not is_ben(edge_param['object_ad']):
                ...
                if not self.is_output_in(edge_output['name'],api_list_dict[source]['is_list']):
                    self.zero_yes_mid_x_list.append(subgraph)
        print(f"Length of zerozero mid x:{len(self.zero_yes_mid_x_list)}")
                
    def zero_yes_mid_x_multi(self): ## mid를 모르는 거고 source가 list를 반환
        for subgraph in tqdm(self.subgraph_list):
            source,target,source_params,target_params,edge=self.node_param_extraction(subgraph)
            if len(source_params) != 0 or len(target_params) <= 1:
                continue
            ...
            if not is_ben(edge_param['object_ad']): ## ben이면 False를 반환함
                ...
                if self.is_output_in(edge_output['name'],api_list_dict[source]['is_list']):
                    self.zero_yes_mid_x_multi_list.append(subgraph)
        print(f"Length of zerozero mid x multi:{len(self.zero_yes_mid_x_multi_list)}")
                
    def fill_form(self):
        for subgraph in self.zero_yes_mid_o_list:
            subgraph = freeformat(self.graph_aug(subgraph))
            subgraph['scenario'] = self.zero_yes_mid_o.__name__
            for action in self.action_seq_o:
                self.freeform.append({"subgraph":subgraph,"action":action})
        for subgraph in self.zero_yes_mid_x_list:
            subgraph = freeformat(self.graph_aug(subgraph))
            subgraph['scenario'] = self.zero_yes_mid_x.__name__
            for action in self.action_seq_x:
                self.freeform.append({"subgraph":subgraph,"action":action})
        for subgraph in self.zero_yes_mid_x_multi_list:
            subgraph = freeformat(self.graph_aug(subgraph))
            subgraph['scenario'] = self.zero_yes_mid_x_multi.__name__
    ...
    def sample_action_seq(self):
        ...
        scen_type_list = [self.zero_yes_mid_o.__name__,self.zero_yes_mid_x.__name__,self.zero_yes_mid_x_multi.__name__]
        random.seed(1)
    ...
    def generate_prompt(self,sampling = False):
        ...
        for idx,subgraph in enumerate(tqdm(freeform_list)):
            ...
            for trg_opt in trg_opt_list:
                scenario = self.scenario_generation(subgraph['subgraph'],subgraph['action'],mock_data,trg_opt,situation)
                if scenario == False:
                    break
                dst = self.dst_generation(subgraph['subgraph'],subgraph['action'],mock_data,trg_opt)
                dst_idx = 0
                for turn_idx,turn in enumerate(scenario):
                    if "System turn\n" in turn:
                        scenario[turn_idx]+=f"\nDialogue state: {dst[dst_idx]}"
                        dst_idx+=1
                # print(f"Cnt of system turn:{dst_idx}, Cnt of dst:{len(dst)}")
                scenario = "\n\n".join(scenario)
    ...
    def dst_generation(self,subgraph,action_seq,mock_data,trg_opt):
        ...
        for action_idx,action in enumerate(action_seq): 
            if action in user_action_list_backward:
                ...
                if action == "request":
                    ...
                    ## list or not ##
                    if action_seq.count('request') == 2 and "multi" not in subgraph['scenario']:
                    ...
                    elif action_seq.count('request') == 2 and "multi" in subgraph['scenario']:
                        request_idx = self.find_all_indexes(action_seq,"request")
                        if action_idx == request_idx[-1]:
                            for param in current_data['data_1']:
                                if param in req_name:
                    ...
                    ## inform intent add 일 경우
                    if 'inform_intent_clear_add' in action_seq:
                        tmp_dst['api_status']['input_parameter'][subgraph['chain_param']] = current_data['data_1'][subgraph['chain_param']]
                    ## end ##
    ...
    def scenario_generation(self,subgraph,action_seq,mock_data,trg_opt,situation):
        prompt=[]
        stage,source,target = "target",subgraph['edge'][0]['source'],subgraph['edge'][0]['target']
        current_api,current_data = target, self.mockdata2value(deepcopy(mock_data['target']))
        
        if len(self.mockdata2value(deepcopy(mock_data['target']))['data_1']) == 0:
            print("Target:",self.mockdata2value(deepcopy(mock_data['target']))['data_1'])
            ...
            if action in user_action_list_backward:
                ...
                if action == "inform":
                    if is_list:
                        ...
                        tmp_situation+=f" User answers only with the values in here:{self.mockdata2value(deepcopy(mock_data[stage]))['data_1']}(Answer only the value asked by the system)"
                if action == "fail_inform":
                    if action_seq.count("request")==2 and "multi" not in subgraph['scenario']: ## list가 아니고 뒤늦게 target param 물어볼 때
                        tmp_situation = user_action_list_backward[action]['template'].format(api=current_api,input_param = subgraph['chain_param'])
                    else: ## 여기선 chain param 제외하곤 정보를 다 준거임
                        param_list = subgraph[stage+"_req"]
                ...
                if action == "request":
                    if action_seq[idx-1]=="call":
                        if "multi" in subgraph['scenario']:## list 일 때 request 해오는 경우
                            is_list=True
                        ...
                        else: # list가 아닌경우, 이는 target을 실행을 위한 input을 user에게 더 물어야 하는 상황임
                            stage,current_api,current_data = "target",target,self.mockdata2value(deepcopy(mock_data['target']))
                            param_name = list(set([param['name'] for param in api_list_dict[current_api]['required_parameters']]) - set([subgraph['chain_param']]))
                            if trg_opt:
                                param_name+=trg_opt['name']
                            tmp_situation = system_action_list_backward[action]["template_tmp"].format(chain_param = subgraph['chain_param'],input_param=param_name)
                    ...
                    elif "inform_intent_clear_add" in action_seq: ## add는 항상 chain param에 대해서 add하도록 함
                        value = current_data['data_1'][subgraph['chain_param']]
                        tmp_situation = system_action_list_backward[action]['template_add'].format(chain_param = subgraph['chain_param'],value=value)
                        req_list = [param for param in subgraph[stage+"_req"] if param['name'] != subgraph['chain_param']]
                        if len(req_list)>0:
                            tmp_situation+=req_prompt.format(input_param = list2string(req_list))
                        if trg_opt:
                            opt_list = [param for param in subgraph[stage+"_opt"] if param['name'] == trg_opt['name']]
                            tmp_situation+=opt_prompt.format(input_param = list2string(opt_list))
                    else: # 이건 맨 처음에 target에 대해서 request 할 때임
                        ...
                        req_list,opt_list = subgraph[stage+'_req'],subgraph[stage+"_opt"]
                        if len(req_list)>0:
                            tmp_situation+=req_prompt.format(input_param = list2string(req_list))
                        if trg_opt:
                            ask_opt = []
                    ...
                    if action_seq[idx-1] == "inform_intent_clear_add": ## add되서 바로 call할 때
                        value = current_data['data_1'][subgraph['chain_param']]
                    ...
                    elif stage == "target" and "fail_inform" not in action_seq:
                        tmp_situation = system_action_list_backward[action]['template'].format(api=current_api)
                    else:
                        if stage == "source":
                            tmp_situation = system_action_list_backward[action]['template_chain'].format(api_s=source,
                            ...
                            tmp_situation+=" (This API doesn't require any input parameters.)"
                        elif "fail_inform" in action_seq: ## source를 갖다오는 경우에만 이렇게 해야 함
                            tmp_situation = system_action_list_backward[action]['template'].format(api=current_api)
                            tmp_situation+=f"Input parameter source: From previous API call results({subgraph['chain_param']})."
                            req_list = [param for param in subgraph[stage+"_req"] if param['name'] != subgraph['chain_param']]
                            opt_list = [param for param in subgraph[stage+"_opt"]]
                            if trg_opt:
                                for opt in opt_list:
                            ...
                            if len(req_list)>0:
                                if action_seq[idx-1] == "call" or "multi" in subgraph['scenario']:
                                    ...
                                    tmp_situation+=f" From the utterence of previous turn of user utterence({list2string_name(req_list)})."
...
try:
    ...
    with get_openai_callback() as cb:
        zeroyes = ZeroYes(pair_list)
    print(cb)

    with open(f"zeroyes/zeroyes_scen_{pair_num}.json",'w') as f:
        json.dump(zeroyes.prompt_list,f,indent=4)
        
    dst_list = []
    for idx in range(len(zeroyes.scenario_list)):
        dst_list.append(zeroyes.scenario_list[idx]['dst'])
        
    with open(f"zeroyes/zeroyes_dst_{pair_num}.json",'w') as f:
        json.dump(dst_list,f,indent=4)
        
        
    with open(f"zeroyes/zeroyes_mockdata_{pair_num}.json",'w') as f:
        json.dump(zeroyes.mockdata_list,f,indent=4)
except Exception as e:
    with open("zeroyes_error.txt",'w') as f:
