...
# with open("pair_list/pair_list.json",'r') as f:
...
user_action_list_backward = { 
    ...
    "inform":
        ...
        "template":"The user provides the requested information to the system. In this turn, the user never answers with a short response, but with utterances of 7 words or more.",
        "template_list":"The user answers the system's request with the value of {output_comp} '{value}' from the {api} call results."},
    "fail_inform":
        {"description":"Fail to reply to the system's request. In this turn, the user says they do not know the answer.",
...
system_action_list_backward = {
    ...
    "request":
        ...
        "template_chain":"Based on the results of the retriever, the system believes the results from API {api_s} can provide the input parameter {chain_param} for API {api_t}. To call {api_s}, the system asks the user for the input parameter. In this turn, the system does not provide an example input parameter to the user."
        },
    "response":
    ...
    "call":
        {"description":"Call the API with the collected information from the user or elsewhere and do not respond to the user yet.",
         "template":"Based on the user's response or previous API call results, the system gathers the input parameter {input_param} and calls API {api}.",
    ...
    "suggest":{
        "description":"Make a suggestion for an unclear user's intent and ask whether it satisfies the user.",
        "template":"Since the user's query is unclear, no API with a retriever score of more than 0.6 has been found. However, there are several APIs with scores between 0.5 and 0.6. The system asks if it would be good to run {api} with the highest score among them and bring the result. At this time, the system does not directly mention the name of the API."
    },
    "retriever_call":{
        "description":"Call the retriever to find the proper API to satisfy the user's requests.",
        "template":"The system that received the user's query calls the retriever to find the appropriate API. In this turn, the system's thought is 'The user seems to have intent. I will call the retriever.'",
...
class ZeroZero(PairClass):
    def __init__(self,subgraph_list):
        ...
        self.action_seq_x = [ ## source를 한 번 거쳐야 함
            ("inform_intent_clear","retriever_call","request","fail_inform","retriever_call","call","call","response","user_bye","system_bye"),
            ("inform_intent_vague","retriever_call","clarify","inform_intent_clear","retriever_call","request","fail_inform","retriever_call","call","call","response","user_bye","system_bye"),
            ("inform_intent_vague","retriever_call","suggest","affirm","request","fail_inform","retriever_call","call","call","response","user_bye","system_bye"),
            ("inform_intent_vague","retriever_call","suggest","negate","response_fail")
        ]
        self.action_seq_x_multi = [ ## source 가 list 반환
            ("inform_intent_clear","retriever_call","request","fail_inform","retriever_call","call","request","inform","call","response","user_bye","system_bye"),
            ("inform_intent_vague","retriever_call","clarify","inform_intent_clear","retriever_call","request","fail_inform","retriever_call","call","request","inform","call","response","user_bye","system_bye"),
            ("inform_intent_vague","retriever_call","suggest","affirm","request","fail_inform","retriever_call","call","request","inform","call","response","user_bye","system_bye"),
            ("inform_intent_vague","retriever_call","suggest","negate","response_fail"),
        ]
        self.zero_zero_mid_o_list = []
        self.zero_zero_mid_x_list = []
        self.zero_zero_mid_x_multi_list = []
        self.freeform = []
        self.prompt_list = []
        self.scenario_list = []
        self.mockdata_list = []
        self.zero_zero_mid_o()
        self.zero_zero_mid_x()
        self.zero_zero_mid_x_multi()
        self.fill_form()
        self.counter()
        self.sports_ratio()
    ...
    def counter(self):
        print("-----------------------")
        print(f"Total scenario cnt: {len(self.freeform)}")
        print("-----------------------")
    ...
    def zero_zero_mid_o(self): ## mid 가 아는 거
        for subgraph in self.subgraph_list:
            ...
            if not is_ben(edge_param['object_ad']):
                continue
            self.zero_zero_mid_o_list.append(subgraph)
        print(f"Length of zerozero mid o:{len(self.zero_zero_mid_o_list)}")
    ...
    def zero_zero_mid_x(self): ## mid 가 모르는 거
        for subgraph in self.subgraph_list:
            source,target,source_params,target_params,edge=self.node_param_extraction(subgraph)
            if len(source_params) != 0 or len(target_params) != 1:
                continue
            ...
            edge_param = {"name":edge[3][0],"object_ad":edge[3][1],"object_long":edge[3][2]}
            if not is_ben(edge_param['object_ad']):
                # if edge_output not in api_list_dict[source]['is_list']:
                if not self.is_output_in(edge_output['name'],api_list_dict[source]['is_list']):
                    self.zero_zero_mid_x_list.append(subgraph)
        print(f"Length of zerozero mid x:{len(self.zero_zero_mid_x_list)}")
                
    def zero_zero_mid_x_multi(self): ## mid를 모르는 거고 source가 list를 반환
        for subgraph in self.subgraph_list:
            source,target,source_params,target_params,edge=self.node_param_extraction(subgraph)
            if len(source_params) != 0 or len(target_params) != 1:
                continue
            ...
            if not is_ben(edge_param['object_ad']): ## ben이면 False를 반환함
                # if edge_output in api_list_dict[source]['is_list']:
                if self.is_output_in(edge_output['name'],api_list_dict[source]['is_list']):
                    self.zero_zero_mid_x_multi_list.append(subgraph)
        print(f"Length of zerozero mid x multi:{len(self.zero_zero_mid_x_multi_list)}")
                
    def fill_form(self):
        for subgraph in self.zero_zero_mid_o_list:
            subgraph = freeformat(self.graph_aug(subgraph))
            subgraph['scenario'] = self.zero_zero_mid_o.__name__
            for action in self.action_seq_o:
                self.freeform.append({"subgraph":subgraph,"action":action})
        for subgraph in self.zero_zero_mid_x_list:
            subgraph = freeformat(self.graph_aug(subgraph))
            subgraph['scenario'] = self.zero_zero_mid_x.__name__
            for action in self.action_seq_x:
                self.freeform.append({"subgraph":subgraph,"action":action})
        for subgraph in self.zero_zero_mid_x_multi_list:
            subgraph = freeformat(self.graph_aug(subgraph))
            subgraph['scenario'] = self.zero_zero_mid_x_multi.__name__
    ...
    def sample_action_seq(self):
        ...
        scen_type_list = [self.zero_zero_mid_o.__name__,self.zero_zero_mid_x.__name__,self.zero_zero_mid_x_multi.__name__]
        random.seed(1)
    ...
    def generate_prompt(self,sampling=False):
        ...
        for idx,subgraph in enumerate(tqdm(freeform_list)):
            # if subgraph['action'] != ("inform_intent_vague","retriever_call","suggest","negate","response_fail"):
            #     continue
            # print(f"Current turn: {idx}")
            ...
            scenario = self.scenario_generation(subgraph['subgraph'],subgraph['action'],mock_data,situation)
            if scenario == False:
                continue
            dst = self.dst_generation(subgraph['subgraph'],subgraph['action'],mock_data)
            dst_idx = 0
    ...
    def dst_generation(self,subgraph,action_seq,mock_data):
    ...
    def scenario_generation(self,subgraph,action_seq,mock_data,situation):
        prompt=[]
        stage,source,target = "target",subgraph['edge'][0]['source'],subgraph['edge'][0]['target']
        current_api,current_data = target, self.mockdata2value(deepcopy(mock_data['target']))
        is_list = False
            ...
            if action in user_action_list_backward:
                ...
                if action == "request":
                    ...
                    if action_seq[idx-1] == "inform_intent_clear_add":
                        ...
                        tmp_situation = system_action_list_backward[action]['template'].format(api=current_api,input_param = subgraph['chain_param'])
                        if stage == "source":
                            stage,current_api,current_data = "target",target,self.mockdata2value(deepcopy(mock_data['target']))
                    tmp_situation = ret_prompt.format(api=current_api) + tmp_situation
                    if action_seq[idx-1] == "fail_inform":
                        tmp_situation = system_action_list_backward[action]['template_fail'].format(api=source,input_param = subgraph['chain_param'],output_comp = subgraph['chain_output'])
...
try:    
    with get_openai_callback() as cb:
        zerozero = ZeroZero(pair_list)
    print(cb)
    print(len(zerozero.freeform))

    with open(f"zerozero/zerozero_scen_{pair_num}.json",'w') as f:
        json.dump(zerozero.prompt_list,f,indent=4)
        
    dst_list = []
    for idx in range(len(zerozero.scenario_list)):
        dst_list.append(zerozero.scenario_list[idx]['dst'])
        
    with open(f"zerozero/zerozero_dst_{pair_num}.json",'w') as f:
        json.dump(dst_list,f,indent=4)
        
        
    with open(f"zerozero/zerozero_mockdata_{pair_num}.json",'w') as f:
        json.dump(zerozero.mockdata_list,f,indent=4)

except Exception as e:
    with open("zerozero_error.txt",'w') as f:
